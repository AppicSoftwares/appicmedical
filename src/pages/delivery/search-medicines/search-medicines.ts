...
export class SearchMedicinesPage {
  ...
  getAddress(postData: any) {
    this.deliveryService.getAddress(postData).then((result) => {
      ...
      if (resultData.status == "success") {
        ...
        this.tempStorage.cart.deliveryInfo.address.longitude =
          postData.longitude;
  ...
  updateDrug() {
    ...
    if (this.currentDrug != null) {
      ...
      if (drugindex > -1) {
        this.cartDrugs[drugindex].drugspec = {
          mg: this.drugmg,
          count: this.drugcount,
        };
        this.setCart();
      }
    }
  ...
  trigger_dosage(gpi10 = "") {
    ...
    this.deliveryService.rxapi_strength(params).then((result) => {
      ...
      if (resultData.predictions.length > 0) {
        ...
        this.drugSelected.QtySelected = {
          ...this.predictions[0].QtyPrediction[0],
        };
        // this.drugSelectedIndex = 0;
      }
      this.addingInProgress = false;
  ...
  trigger_strength() {
    ...
    /* Muni uncoment and see
        ...
        Muni uncoment and see
        */
    let GPI14s_array: any = this.strengthData.value[
      this.filterDataForm.value.strength
    ].data.GPI14s;
    ...
    this.deliveryService.rxapi_package(params).then((result) => {
      ...
      if (resultData.drugs.length > 0) {
        for (let s = 0; s <= resultData.drugs.length - 1; s++) {
          ...
          if (
            item.Strength ==
            this.strengthData.value[this.filterDataForm.value.strength].data
              .Strength
          ) {
            if (this.packageData.index.indexOf(item.LN) < 0) {
              this.packageData.index.push(item.LN);
              var newObj = {
                text: item_name_lowercase,
                value: this.packageData.index.indexOf(item.LN),
                data: [],
              };
              ...
              this.packageData.value[
                this.packageData.index.indexOf(item.LN)
              ] = newObj;
            }
            this.packageData.value[
              this.packageData.index.indexOf(item.LN)
  ...
  trigger_package() {
    ...
    packageDetails = this.packageData.value[this.filterDataForm.value.package]
      .data;
    ...
    if (packageDetails.length > 0) {
      ...
      for (let pd = 0; pd <= packageDetails.length - 1; pd++) {
        ...
        if (this.packagedescData.index.indexOf(item.PackageDesc) < 0) {
          ...
          var newObj = {
            text: item_name_lowercase,
            value: this.packagedescData.index.indexOf(item.PackageDesc),
            data: [],
          };
          this.packagedescData.value[
            this.packagedescData.index.indexOf(item.PackageDesc)
          ] = newObj;
        }
        this.packagedescData.value[
          this.packagedescData.index.indexOf(item.PackageDesc)
  ...
  trigger_packagedesc() {
    ...
    quantityDetails = this.packagedescData.value[
      this.filterDataForm.value.packagedesc
    ].data;
      ...
      for (let q = 0; q <= quantityDetails.length - 1; q++) {
        ...
        if (this.quantityData.index.indexOf(item_name) < 0) {
          ...
          let newObj = {
            ...
            value: this.quantityData.index.indexOf(item_name),
            ndc: [],
            data: [],
          };
          this.quantityData.value[
            this.quantityData.index.indexOf(item_name)
          ] = newObj;
        }
        this.quantityData.value[
          this.quantityData.index.indexOf(item_name)
  ...
  addToMedications() {
    ...
    this.tempStorage.cart.medications.byNdc[
      this.drugSelected.NDC
    ] = this.drugSelected;
    ...
    this.deliveryService.ajaxDrugs({ term: keyword }).then((result) => {
      ...
      if (resultData.drugs.length > 0) {
        for (var i = 0; i < resultData.drugs.length; i++) {
          ...
          let indexMatched: any = this.deliveryService.contains(
            this.autocomplete,
            item.GPI10s[0],
            "GPI10s",
            true
          ...
          if (indexMatched >= 0) {
            this.deliveryService.contains(
              this.autocomplete[indexMatched]["BN"],
              item.BN
            )
              ? ""
              : this.autocomplete[indexMatched]["BN"].push(item.BN);

            this.deliveryService.contains(
              this.autocomplete[indexMatched]["AN"],
              item.AN
            )
              ? ""
              : this.autocomplete[indexMatched]["AN"].push(item.AN);

            this.autocomplete[indexMatched]["brand_text"] =
              this.autocomplete[indexMatched]["AN"].length > 0
                ? item.DrugTypes == "B"
                  ? "Brand for " +
                    this.autocomplete[indexMatched]["AN"].join(",")
                  : "Gneric for " +
                    this.autocomplete[indexMatched]["AN"].join(",")
                : "";
          } else {
    ...
    this.deliveryService.pharmacyList(this.drugSelected).then((result) => {
      ...
      if (resultData.outdata.length > 0) {
        for (var x = 0; x <= resultData.outdata.length - 1; x++) {
          ...
          let currentIndex = this.cartDrugsKey.indexOf(
            resultData.outdata[x].drug.value
          );
          if (currentIndex > -1) {
            if (
              this.cartDrugs[currentIndex]["pharmacy_id"] ==
              resultData.outdata[x].pharmacy_id
            ) {
              item.selected = true;
          ...
          if (
            this.pharmacySelected &&
            this.pharmacySelected.value !== undefined
          ) {
  ...
  showConfirm() {
    ...
    if (profilemembership == "premiumactive") {
      ...
      if (this.tempStorage.cart.membership.status == "member") {
        ...
        this.deliveryService
          .mobiconfirm(
            "Sign up to create discount card to avail discount up to 80% on prescription medicines.",
      ...
      if (this.tempStorage.cart.membership.status == "member") {
        ...
        this.deliveryService
          .mobiconfirm(
            "Sign up and get more discount for Generic and Prescription Medicines.",
            "Become a member"
          )
          .then((result) => {
            console.log(result);
  ...
  addDrug(pharmacy: any, index: any) {
    ...
    if (
      this.cartDrugsKey.indexOf(pharmacy.drug.value) > -1 ||
      (this.cartPharmacy != null && this.cartPharmacy != pharmacy.pharmacy_id)
    ) {
      this.alertCannotAdd(pharmacy, index);
    } else {
      this.pharmacies[index].selected = true;
      this.pharmacies[index].drugspec = {
        mg: this.drugmg,
        count: this.drugcount,
      };
  ...
  getCart() {
    ...
    this.myLocationObj = {
      latitude: this.tempStorage.cart.deliveryInfo.address.latitude,
      longitude: this.tempStorage.cart.deliveryInfo.address.longitude,
      zip: this.tempStorage.cart.deliveryInfo.address.zip,
    };
  ...
  goto(action: any) {
    if (action == "druginfo") {
      ...
      if (
        ...
        this.tempStorage.cart.pharmacy.pricing.length ==
          this.tempStorage.cart.medications.drugs.length
      ) {
        this.searchDataForm.patchValue({ term: "" });
  ...
  openPharmacyModel() {
    ...
    const modal = this.modalCtrl.create(customAutoComplete, {
      ismodel: true,
      myLocationObj: this.myLocationObj,
      template: "pharmacy_price_list",
      placeHolder: "Search Pharmacy",
      templateBasedData: this.medications,
    });
    modal.onDidDismiss((data) => {
      // console.log(data);
      // console.log(this.tempStorage.cart);
      if (data && data.pricing !== undefined) {
        // means it has pahrmacy data selected
        this.pharmacySelected = data;
        ...
        this.deliveryService
          .mobiToast("Setting your pharmacy, Please wait ...", "success")
          .then((result) => {});
        this.deliveryService.commonUsecase(filterObj).then((result: any) => {
          if (result.data !== undefined) {
            ...
            this.deliveryService
              .mobiToast(
                "Sorry technical error, unable to move forward ...",
                "danger"
              )
              .then((result) => {});
          }
